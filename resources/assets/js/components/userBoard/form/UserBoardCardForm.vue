...
<el-dialog :title="title" v-model="formNewCard">
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="Member" name="member">
          ...
          :members="formInputs.members_card"
          :load-member-url="loadMemberCardUrl +'/' + formInputs.id +'/member'"
          :save-member-url="saveMemberCardUrl +'/' + formInputs.id +'/member'"
          @load="update">
        </user-board-form-member>
        </el-tab-pane>
    </el-tabs>
...
export default {
    props: {
      formInputs: null,
      loadMemberCardUrl: String,
      saveMemberCardUrl: String,
      card: null,
    },
    data() {
        return {
    ...
    computed: {
        filteredChecklists: function() {
            if (this.formInputs.edit === 1) {
                this.dataNewCard.checklists = this.formInputs.checklists
            }
    ...
    methods: {
        ...
        checkEdit: function() {
            if (this.formInputs.edit === 1) {
                this.title = this.formInputs.name
                var startdate = new Date(this.formInputs.startdate);
                var duedate = new Date(this.formInputs.duedate);
        ...
        fetchChecklist: function() {
            if (this.formInputs.edit === 1) {
                var checklists = this.formInputs.checklists
            } else {
                var checklists = []
            }
            return checklists
